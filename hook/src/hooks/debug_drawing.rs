...
pub fn hooks() -> &'static [(&'static str, ExecFn)] {
    &[
        (
...
unsafe fn draw_lines(batcher: NonNull<ULineBatchComponent>, lines: &[FBatchedLine]) {
    if let Some((last, lines)) = lines.split_last() {
        ...
        batched_lines.extend_from_slice(lines);
...
unsafe fn arg<T: Default + Sized>(stack: &mut ue::kismet::FFrame) -> T {
    let mut value = Default::default();
    ue::kismet::arg(stack, &mut value);
    value
}
...
unsafe extern "system" fn exec_draw_debug_line(
    ...
    let start: FVector = arg(stack);
    let end: FVector = arg(stack);
    let color: FLinearColor = arg(stack);
    let duration: f32 = arg(stack);
    let thickness: f32 = arg(stack);
...
unsafe extern "system" fn exec_draw_debug_circle(
    ...
    let world_context: Option<NonNull<ue::UObject>> = arg(stack);
    let center: FVector = arg(stack);
    let radius: f32 = arg(stack);
    let num_segments: u32 = arg(stack);
    let color: FLinearColor = arg(stack);
    let duration: f32 = arg(stack);
    let thickness: f32 = arg(stack);
    let y_axis: FVector = arg(stack);
    let z_axis: FVector = arg(stack);
    let draw_axis: bool = arg(stack);
...
unsafe extern "system" fn exec_draw_debug_cone(
    ...
    let world_context: Option<NonNull<ue::UObject>> = arg(stack);
    let origin: FVector = arg(stack);
    let direction: FVector = arg(stack);
    let length: f32 = arg(stack);
    let angle_width: f32 = arg(stack);
    let angle_height: f32 = arg(stack);
    let num_sides: u32 = arg(stack);
    let color: FLinearColor = arg(stack);
    let duration: f32 = arg(stack);
    let thickness: f32 = arg(stack);
...
unsafe extern "system" fn exec_draw_debug_capsule(
    ...
    let world_context: Option<NonNull<ue::UObject>> = arg(stack);
    let center: FVector = arg(stack);
    let half_height: f32 = arg(stack);
    let radius: f32 = arg(stack);
    let rotation: FRotator = arg(stack);
    let color: FLinearColor = arg(stack);
    let duration: f32 = arg(stack);
    let thickness: f32 = arg(stack);
...
unsafe extern "system" fn exec_tick(
    ...
    let _delta_seconds: f32 = arg(stack);
